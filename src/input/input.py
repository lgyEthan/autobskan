...
def parse_input(raw_options): # 파일 인풋 형식에 따라 구분해둠
    ...
    option_dict = {}
    for i in ["MODE", "METHOD", "CMAP", "CONTRAST", "BSKAN", "VASP", "BLUR_METHOD", "CURRENT", "EXT", "POSCAR", "RADIUS_TYPE", "ATOM_ADDINFO"]:
    ...
    for i in ["BRIGHTNESS", "GAMMA"]: # float값만 받는 경우
        if i in raw_options:
    ...
    def __init__(self, filename="bskan.in"):
        ...
        if type(filename) in [dict, str]:
            ...
            if "CONTRAST" in self.options:
                tmp = self.options["CONTRAST"].upper()
                if tmp.startswith("LI"):
                    self.contrast = "LINEAR"
                elif tmp.startswith("EX"):
                    self.contrast = "EXPONENTIAL"
                elif tmp.startswith("LO"):
                    self.contrast = "LOGARITHM"
                else:
                    raise IOError("input contrast method is not supported. Choose among Linear/Exponential/Logarithm")
